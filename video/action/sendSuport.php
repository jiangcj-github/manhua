...
//時間間隔檢查
$stmt=$conn->prepare("select suport from user_strict_v_cm where nick=? and cid=?");
...
$stmt->close();
$stmt=$conn->prepare("select count(suport) from user_strict_v_cm where nick=? and suport>?");
...
$stmt->close();
//插入頂
$stmt=$conn->prepare("update video_comment set suport=suport+1 where id=? and vid=?");
