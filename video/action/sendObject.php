...
if( $stmt->fetch()&& $stri_sec){
    ...
    if($cur_sec-$stri_sec<60*60*24){
        die_json(["msg"=>"已經踩過了，需等待約".round((60*60*24-$cur_sec+$stri_sec)/3600)."小時"]);
    }
}
$stmt->close();
...
$stmt->close();
//記錄操作時間
$stmt=$conn->prepare("insert into user_strict_v_cm(nick,cid,object) values(?,?,?) ON DUPLICATE KEY update object=?");
